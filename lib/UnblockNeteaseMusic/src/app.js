...
config.port = (config.port || global.userConfig.port).split(':').map(string => parseInt(string))
...
if (config.matchOrder || global.userConfig.source) {
	const provider = new Set(['netease', 'qq', 'xiami', 'baidu', 'kugou', 'kuwo', 'migu', 'joox', 'youtube'])
	const candidate = config.matchOrder || global.userConfig.source
